...
namespace guarneri{
	...
	public:
		...
		void initialize() {
			...
			world_debug_cam = std::move(camera::create(float3::ONE * debug_cam_distance, window().aspect, 60.0f, 0.5f, 10.0f, projection::perspective));
		...
		void draw_world_coords() {
			...
			world_debug_cam->position = float3::ONE * debug_cam_distance;
