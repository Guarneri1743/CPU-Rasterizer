...
/// <returns></returns>
std::vector<Triangle> Clipping::cvv_clipping(const float& near_plane, const tinymath::Frustum& cvv, const Vertex& c1, const Vertex& c2, const Vertex& c3)
{
...
void Clipping::screen_clipping(Vertex& lhs, Vertex& rhs, const size_t& width)
{
...
/// <param name="c3">vertex in clip space</param>
/// <returns></returns>
bool Clipping::inside_cvv(const tinymath::vec4f& c1, const tinymath::vec4f& c2, const tinymath::vec4f& c3)
{
...
/// <returns></returns>
bool Clipping::inside_cvv(const tinymath::vec4f& v)
{
	// z: [-w, w](GL) [0, w](DX)
	// x: [-w, w]
	// y: [-w, w]
...
bool Clipping::backface_culling_ndc(const tinymath::vec3f& c1, const tinymath::vec3f& c2, const tinymath::vec3f& c3)
{
	// front face: ndv >= 0
	// back face: ndv < 0
	auto seg1 = c2 - c1;
...
bool Clipping::frustum_culling_sphere(const tinymath::Frustum& frustum, const tinymath::Sphere& bounding_sphere)
{
	for (int i = 0; i < 6; i++)
	{
...
bool Clipping::conservative_frustum_culling(const tinymath::Frustum& frustum, const Vertex& v1, const Vertex& v2, const Vertex& v3)
{
	for (int i = 0; i < 6; i++)
	{
		auto& plane = frustum[i];
