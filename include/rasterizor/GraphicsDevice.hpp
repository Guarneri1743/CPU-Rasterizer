...
namespace Guarneri
{
	...
	private:
		...
		std::unique_ptr<RawBuffer<tinymath::color_rgba>> framebuffer;
		...
		std::unique_ptr<RawBuffer<tinymath::color_rgba>> msaa_colorbuffer;
		// shadowmap
	...
	public:
		...
		tinymath::color_rgba* get_framebuffer() { size_t size;  return framebuffer->get_ptr(size); };
	...
	private:
		...
		void process_subsamples(RawBuffer<tinymath::color_rgba>* fbuf, RawBuffer<float>* zbuf, RawBuffer<uint8_t>* stencilbuf, const size_t& row, const size_t& col, const Shader& shader, const Vertex& v0, const Vertex& v1, const Vertex& v2, const float& area);
		void process_fragment(RawBuffer<tinymath::color_rgba>* fbuf, RawBuffer<float>* zbuf, RawBuffer<uint8_t>* stencilbuf, const Vertex& v, const size_t& row, const size_t& col, const Shader& shader);
		bool validate_fragment(const PerSampleOperation& op_pass) const;
		bool perform_stencil_test(RawBuffer<uint8_t>* stencilbuf, const uint8_t& ref_val, const uint8_t& read_mask, const CompareFunc& func, const size_t& row, const size_t& col) const;
		void update_stencil_buffer(RawBuffer<uint8_t>* stencilbuf, const size_t& row, const size_t& col, const PerSampleOperation& op_pass, const StencilOp& stencil_pass_op, const StencilOp& stencil_fail_op, const StencilOp& stencil_zfail_op, const uint8_t& ref_val) const;
		bool perform_depth_test(RawBuffer<float>* zbuf, const CompareFunc& func, const size_t& row, const size_t& col, const float& z) const;
